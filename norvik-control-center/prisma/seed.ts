...
async function main() {
  ...
  if ((await prisma.tarea.count()) === 0) {
    ...
    console.log(`✔ ${TAREAS.length} tareas del checklist maestro cargadas.`);
  }

  if ((await prisma.tareaDiaria.count()) === 0) {
    await prisma.tareaDiaria.createMany({
      data: TAREAS_DIARIAS.map((tarea, indice) => ({
        ...tarea,
        orden: indice,
      })),
    });
    console.log(`✔ ${TAREAS_DIARIAS.length} tareas del checklist diario cargadas.`);
  }
  ...
  if ((await prisma.producto.count()) === 0) {
    await prisma.producto.createMany({
      data: PRODUCTOS_SIN_STOCK.map((nombre) => ({
        ...
        stock: 0,
        estado: "activo",
        notas: "Detectado con stock 0. Falta rellenar coste de Trendsi y precio de venta.",
      })),
    });
    console.log(`✔ ${PRODUCTOS_SIN_STOCK.length} productos con stock 0 cargados.`);
  }
  ...
  if ((await prisma.gasto.count()) === 0) {
    await prisma.gasto.create({
      data: {
        ...
        importe: 0,
        recurrente: true,
        notas: "Pon aquí el importe exacto que te cobra Shopify cada mes.",
      },
    });
  }
}
